...
| 社内耐用年数 | 月数または年数（前提5参照：社内基準を使用） |
| 配賦先コストセンターコード | コストセンターマスタを参照。利用部門への按分用 |
| 勘定科目コード | 勘定科目別集計用 |
| 備考 | |

**自動計算項目**：月次減価償却費（取得価額÷社内耐用年数）、償却終了月。期末帳簿価額はBS予算が出力対象外のため算出しない（前提14）。
...
以下は契約マスタ・固定資産マスタ・組織要員マスタのいずれにも当てはまらず、従来どおりの見積もり・積み上げ方式で予算化する。
