...
マスタデータ(品目・設備・工程コード等)は、ERP・MES・OTがそれぞれ別目的でマスタを持つため、同じ物理的実体が複数のIDで管理される「多重管理」が構造的に発生する。これに対処するには、ブリッジ・変換機能を作るだけでは不十分で、変換ロジックの正しさを継続的に担保する責任者(オーナー)を明文化する必要がある。
